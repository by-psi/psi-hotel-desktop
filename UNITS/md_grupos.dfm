object F_grupos: TF_grupos
  Left = 428
  Top = 103
  Caption = 'psi-hotel - cadastro de grupos (produtos)'
  ClientHeight = 491
  ClientWidth = 597
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 8
    Width = 35
    Height = 13
    Caption = 'GRUPO'
    FocusControl = ed_idgrupo
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label2: TLabel
    Left = 71
    Top = 8
    Width = 159
    Height = 13
    Caption = 'DESCRI'#199#195'O DAS MERCADORIAS'
    FocusControl = ed_grupo
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object bt_incluir: TSpeedButton
    Left = 163
    Top = 431
    Width = 42
    Height = 49
    Hint = 'ADICIONAR NOVO'
    Glyph.Data = {
      
      6363636363636363636363696969979797FFFFFFFFFFFFFFFFFF}
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = processar_evento
  end
  object bt_excluir: TSpeedButton
    Left = 211
    Top = 431
    Width = 42
    Height = 49
    Hint = 'EXCLUIR'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = processar_evento
  end
  object bt_imprimir: TSpeedButton
    Left = 259
    Top = 431
    Width = 42
    Height = 49
    Hint = 'IMPRIMIR'
    Glyph.Data = {
      
      00000000000000000000008D8D8DFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = processar_evento
  end
  object bt_ok: TSpeedButton
    Left = 323
    Top = 431
    Width = 78
    Height = 49
    Hint = 'CONFIRMAR'
    Caption = 'OK'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = processar_evento
  end
  object bt_cancelar: TSpeedButton
    Left = 407
    Top = 431
    Width = 132
    Height = 49
    Hint = 'CANCELAR'
    Caption = 'CANCELAR'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = processar_evento
  end
  object bt_fechar: TSpeedButton
    Left = 545
    Top = 431
    Width = 42
    Height = 49
    Hint = 'REVISAR CONTA'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    ParentShowHint = False
    ShowHint = True
    Transparent = False
    OnClick = processar_evento
  end
  object ed_idgrupo: TDBEdit
    Left = 8
    Top = 24
    Width = 57
    Height = 21
    Color = 13303754
    DataField = 'ID_GRUPO'
    DataSource = ds_grupos
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 0
  end
  object ed_grupo: TDBEdit
    Left = 72
    Top = 24
    Width = 273
    Height = 21
    CharCase = ecUpperCase
    DataField = 'GRUPO'
    DataSource = ds_grupos
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
  end
  object Grid_1: TDBGrid
    Left = 8
    Top = 64
    Width = 579
    Height = 361
    DataSource = ds_grupos
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnTitleClick = Grid_1TitleClick
    Columns = <
      item
        Expanded = False
        FieldName = 'ID_GRUPO'
        Title.Caption = 'Grupo'
        Width = 51
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'GRUPO'
        Title.Caption = 'Descri'#231#227'o das Mercadorias'
        Width = 270
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CHV'
        Width = 29
        Visible = True
      end>
  end
  object DBNavigator_1: TDBNavigator
    Left = 9
    Top = 432
    Width = 148
    Height = 48
    DataSource = ds_grupos
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    Hints.Strings = (
      'IR P/ INICIO'
      'CONTA ANTERIOR'
      'PR'#211'XIMA CONTA'
      'IR P/ FINAL '
      'INSERIR CONTA'
      'EXCLUIR CONTA'
      'EDITAR CONTA'
      'GRAVAR ALTERA'#199#213'ES'
      'CANCELAR ALTERA'#199#213'ES'
      'ATUALIZAR')
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
  end
  object ds_grupos: TDataSource
    DataSet = dm_firedac.T_grupos
    Left = 544
    Top = 8
  end
end
